# 计费与权益（Billing and Entitlements）

## Purpose

本 spec 定义 LS 的「计费与权益」能力（`billing-and-entitlements`），用于规范 权益校验、用量计量、账单生成与争议审计 的契约、门禁与审计。
关键产物包括：权益定义、用量事件、计费记录、争议证据链。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「计费与权益」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `org_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（权益定义、用量事件、计费记录、争议证据链）定义字段级约束，并对完整性/引用可解析性实施校验。
