# 商业化就绪门禁 — 审计日志（字段/脱敏/导出结构）

本文件定义 `commercial-readiness-gates` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | GATE_EVALUATE/CHECKLIST_UPDATE/APPROVAL_DECISION/EVIDENCE_ATTACH/AUDIT_EXPORT |
| `version_id` | 是 | 版本标识 |
| `checklist_id` | 否 | 清单标识（若适用） |
| `approval_id` | 否 | 审批标识（若适用） |
| `evidence_id` | 否 | 许可证据标识（若适用） |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、合同/客户信息等）。
