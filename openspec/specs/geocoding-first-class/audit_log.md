# 地理编码一等公民 — 审计日志（字段/脱敏/导出结构）

本文件定义 `geocoding-first-class` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | GEO_REQUEST/GEO_ENTITY_COMMIT/FALLBACK_TRIGGERED/METRICS_REPORT_GENERATE/AUDIT_EXPORT |
| `user_id` | 是 | 用户标识（隔离边界） |
| `org_id` | 否 | 组织标识（若适用） |
| `version_id` | 是 | 版本标识 |
| `geo_entity_id` | 否 | 地理实体标识（若适用） |
| `precision` | 否 | 精度枚举（若适用） |
| `strategy` | 否 | 回退策略（若适用） |
| `result` | 是 | PASS/DENY/BLOCK/FALLBACK/ERROR |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、原始地址文本、完整位置描述等）。
