# 地理编码一等公民（Geocoding First Class）

## Purpose

本 spec 定义 LS 的「地理编码一等公民」能力（`geocoding-first-class`），用于规范 地理编码实体化、归一化、精度/置信度与回退策略 的契约、门禁与审计。
关键产物包括：标准化地理实体、精度/置信度元数据、回退记录、指标报表。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「地理编码一等公民」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`, `user_id`）及其语义边界。
...
系统 MUST 对关键产物（标准化地理实体、精度/置信度元数据、回退记录、指标报表）定义字段级约束，并对完整性/引用可解析性实施校验。
...
系统 SHALL 定义可执行的验收口径，且未达标 MUST 阻断进入其所属门禁（Gate-3）。
