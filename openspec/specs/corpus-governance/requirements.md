# 语料治理（Corpus Governance）

## Purpose

本 spec 定义 LS 的「语料治理」能力（`corpus-governance`），用于规范 语料资产的所有权、许可、版本、血缘与退役策略 的契约、门禁与审计。
关键产物包括：资产登记、许可与使用约束、版本与血缘、退役记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 资产契约与范围（Scope & Contract）
系统 MUST 为「语料治理」定义资产范围与契约边界，并明确最小标识符集合（至少包含 `asset_id`, `corpus_release_id`, `version_id`）。
