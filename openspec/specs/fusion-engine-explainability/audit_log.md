# 融合引擎可解释性 — 审计日志（字段/脱敏/导出结构）

本文件定义 `fusion-engine-explainability` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | EXPLAIN_REQUEST/EXPLANATION_COMMIT/CITATION_RESOLVE/DEVIATION_DETECTED/AUDIT_EXPORT |
| `user_id` | 否 | 用户标识（若适用；隔离边界） |
| `org_id` | 否 | 组织标识（若适用） |
| `engine_id` | 是 | 引擎标识 |
| `version_id` | 是 | 版本标识 |
| `corpus_release_id` | 是 | 语料发布标识 |
| `decision_id` | 否 | 融合决策标识（若适用） |
| `explanation_id` | 否 | 解释记录标识（若适用） |
| `chain_id` | 否 | 证据链标识（若适用） |
| `result` | 是 | PASS/DENY/BLOCK/ERROR |
| `rejection_reason_code` | 否 | 拒绝/阻断原因代码（见契约枚举） |
| `deviation_type` | 否 | 偏差类型（若适用） |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、完整解释内容、原始引用片段等）。
