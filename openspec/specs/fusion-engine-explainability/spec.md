# 融合引擎可解释性（Fusion Engine Explainability）

## Purpose

本 spec 定义 LS 的「融合引擎可解释性」能力（`fusion-engine-explainability`），用于规范 融合决策的解释输出、证据对齐与可复现回放 的契约、门禁与审计。
关键产物包括：解释记录、证据对齐引用、置信度/不确定性、回放输入集。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「融合引擎可解释性」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `engine_id`, `version_id`, `corpus_release_id`）及其语义边界。
...
系统 MUST 对关键产物（解释记录、证据对齐引用、置信度/不确定性、回放输入集）定义字段级约束，并对完整性/引用可解析性实施校验。
