# 生产默认值（禁止Mock）（Production Defaults No Mock）

## Purpose

本 spec 定义 LS 的「生产默认值（禁止Mock）」能力（`production-defaults-no-mock`），用于规范 生产默认值、环境隔离与禁止 mock/静默降级 的契约、门禁与审计。
关键产物包括：配置校验规则、环境隔离约束、启动失败原因记录、违规审计。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「生产默认值（禁止Mock）」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`）及其语义边界。
...
系统 MUST 对关键产物（配置校验规则、环境隔离约束、启动失败原因记录、违规审计）定义字段级约束，并对完整性/引用可解析性实施校验。
