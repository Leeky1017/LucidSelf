# 文本归一化与对齐 — 审计日志（字段/脱敏/导出结构）

本文件定义 `text-normalization-alignment` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | RULESET_DEFINE/UPDATE, ALIGNMENT_MAP_DEFINE/UPDATE, TRANSFORM, COVERAGE_REPORT, EXCEPTION_LIST, AUDIT_EXPORT |
