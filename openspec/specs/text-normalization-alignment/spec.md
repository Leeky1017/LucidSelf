# 文本归一化与对齐（Text Normalization Alignment）

## Purpose

本 spec 定义 LS 的「文本归一化与对齐」能力（`text-normalization-alignment`），用于规范 典籍与多源文本的归一化规则、对齐策略与可追溯变换 的契约、门禁与审计。
关键产物包括：归一化规则集、对齐映射、变换审计记录、覆盖率与例外清单。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 资产契约与范围（Scope & Contract）
系统 MUST 为「文本归一化与对齐」定义资产范围与契约边界，并明确最小标识符集合（至少包含 `corpus_release_id`, `version_id`）。
