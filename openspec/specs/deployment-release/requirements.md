...
本 spec 定义 LS 的「部署与发布」能力（`deployment-release`），用于规范 环境晋级、发布验证与回滚流程 的契约、门禁与审计。
关键产物包括：发布产物清单、晋级记录、发布验证报告、回滚记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「部署与发布」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`）及其语义边界。
...
系统 MUST 对关键产物（发布产物清单、晋级记录、发布验证报告、回滚记录）定义字段级约束，并对完整性/引用可解析性实施校验。
