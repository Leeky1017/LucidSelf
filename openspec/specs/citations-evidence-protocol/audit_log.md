# 引用与证据协议 — 审计日志（字段/脱敏/导出结构）

本文件定义 `citations-evidence-protocol` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | EVIDENCE_CHAIN_VALIDATE/CREATE, CITATION_ANCHOR_RESOLVE, AUDIT_EXPORT |
| `chain_id` | 否 | 关联证据链标识（若适用） |
| `anchor_id` | 否 | 关联引用锚点标识（若适用） |
| `asset_id` | 否 | 关联资产标识（若适用） |
...
- 对外导出时 MUST 保留可定位问题所需的最小字段：`audit_id`, `created_at`, `event_type`, `version_id`, `corpus_release_id`, `result`, `rejection_reason_code`（若适用）。
