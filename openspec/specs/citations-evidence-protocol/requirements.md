# 引用与证据协议（Citations and Evidence Protocol）

## Purpose

本 spec 定义 LS 的「引用与证据协议」能力（`citations-evidence-protocol`），用于规范 evidence_chain 与 citation_anchor 的生成、校验与审计导出 的契约、门禁与审计。
关键产物包括：evidence_chain、citation_anchor、provenance 记录、审计导出包。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「引用与证据协议」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `corpus_release_id`, `version_id`, `user_id`）及其语义边界。
...
系统 MUST 对关键产物（evidence_chain、citation_anchor、provenance 记录、审计导出包）定义字段级约束，并对完整性/引用可解析性实施校验。
