...
本 spec 定义 LS 的「安全/隐私/合规」能力（`security-privacy-compliance`），用于规范 数据分类、访问控制、加密、隐私请求与事件响应 的契约、门禁与审计。
关键产物包括：数据分类标签、访问控制证据、隐私请求处理记录、事件响应记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「安全/隐私/合规」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `org_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（数据分类标签、访问控制证据、隐私请求处理记录、事件响应记录）定义字段级约束，并对完整性/引用可解析性实施校验。
