# 语义层核心 — 审计日志（字段/脱敏/导出结构）

本文件定义 `semantic-layer-core` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | SEMANTIC_ENTRY_WRITE/UPDATE, SEMANTIC_QUERY, ANCHOR_BIND, AUDIT_EXPORT |
| `semantic_entry_id` | 否 | 关联语义条目（若适用） |
