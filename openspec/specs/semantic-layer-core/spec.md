# 语义层核心（Semantic Layer Core）

## Purpose

本 spec 定义 LS 的「语义层核心」能力（`semantic-layer-core`），用于规范 语义条目(semantic entry)的数据模型、持久化与查询契约 的契约、门禁与审计。
关键产物包括：semantic entry、索引、provenance、citation_anchor 绑定、查询响应。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「语义层核心」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `engine_id`, `user_id`, `version_id`, `corpus_release_id`）及其语义边界。
...
系统 MUST 对关键产物（semantic entry、索引、provenance、citation_anchor 绑定、查询响应）定义字段级约束，并对完整性/引用可解析性实施校验。
