# Golden Set与回归评测 — 审计日志（字段/脱敏/导出结构）

本文件定义 `golden-sets-and-regression` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | GOLDEN_SET_DEFINE/UPDATE, COVERAGE_REPORT, THRESHOLD_DEFINE/UPDATE, REGRESSION_RUN, DRIFT_ALERT |
| `golden_set_id` | 否 | 关联 golden set 标识（若适用） |
| `version_id` | 是 | 关联版本标识 |
| `corpus_release_id` | 是 | 语料发布版本标识 |
| `engine_id` | 是 | 关联引擎标识（若适用） |
...
- 对外导出时 MUST 保留可定位问题所需的最小字段：`audit_id`, `created_at`, `event_type`, `version_id`, `corpus_release_id`, `engine_id`, `result`, `rejection_reason_code`（若适用）。
