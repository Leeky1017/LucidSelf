# Golden Set与回归评测（Golden Sets and Regression）

## Purpose

本 spec 定义 LS 的「Golden Set与回归评测」能力（`golden-sets-and-regression`），用于规范 golden set 的版本化、覆盖率与回归阈值 的契约、门禁与审计。
关键产物包括：golden set 清单、覆盖率报告、回归阈值与结果、漂移告警。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「Golden Set与回归评测」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`, `corpus_release_id`, `engine_id`）及其语义边界。
...
系统 MUST 对关键产物（golden set 清单、覆盖率报告、回归阈值与结果、漂移告警）定义字段级约束，并对完整性/引用可解析性实施校验。
