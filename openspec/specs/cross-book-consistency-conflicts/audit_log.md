# 跨书一致性与冲突 — 审计日志（字段/脱敏/导出结构）

本文件定义 `cross-book-consistency-conflicts` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | CONFLICT_DETECT, DECISION_WRITE, EXCEPTION_APPROVE, REPORT_EXPORT, AUDIT_EXPORT |
