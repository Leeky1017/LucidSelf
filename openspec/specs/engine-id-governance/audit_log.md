# 引擎ID治理 — 审计日志（字段/脱敏/导出结构）

本文件定义 `engine-id-governance` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_id` | 是 | 关联生命周期事件标识 |
| `event_type` | 是 | REGISTER/UPDATE/STATUS_CHANGE/DEPRECATE/REPLACE/UNREGISTER |
| `engine_id` | 是 | 关联引擎标识 |
| `version_id` | 是 | 关联版本标识 |
...
- 对外导出时 MUST 保留可定位问题所需的最小字段：`audit_id`, `created_at`, `event_id`, `engine_id`, `version_id`, `result`, `rejection_reason_code`（若适用）。
