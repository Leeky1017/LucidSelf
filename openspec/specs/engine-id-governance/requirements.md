# 引擎ID治理（Engine ID Governance）

## Purpose

本 spec 定义 LS 的「引擎ID治理」能力（`engine-id-governance`），用于规范 engine_id 的注册、唯一性与生命周期治理 的契约、门禁与审计。
关键产物包括：engine_id 注册表、生命周期变更记录、弃用/替代关系、审计记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「引擎ID治理」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `engine_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（engine_id 注册表、生命周期变更记录、弃用/替代关系、审计记录）定义字段级约束，并对完整性/引用可解析性实施校验。
