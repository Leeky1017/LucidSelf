...
- `PYTHONPATH=. .venv/bin/pytest backend/tests/gate0/test_engine_id_governance.py -q`（本 spec 工程验收：关键路径 engine_id 治理）
- `openspec list --specs`（确认本 spec 的任务进度可见）
 - `.venv/bin/python -c 'import json; json.load(open(\"openspec/specs/engine-id-governance/contract/engine_id_governance_contract.v1.json\")); print(\"OK\")'`（契约机器可读校验）
...
## 必须产物（最低集合）
- `openspec/specs/engine-id-governance/spec.md`
- `openspec/specs/engine-id-governance/requirements.md`
- `openspec/specs/engine-id-governance/tasks.md`
