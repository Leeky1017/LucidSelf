# Spec: ls-delivery-workflow

## Purpose

Define LucidSelf (LS) collaboration and delivery workflow as enforceable gates (Issue → PR → Checks → Auto-merge), so changes remain spec-first, auditable, and reproducible.

## Requirements

### Requirement: Every change is Issue-gated and PR-delivered

All changes MUST be tracked by a GitHub Issue `#N` and MUST be merged via a PR that follows LS delivery hard gates:
...
### Requirement: PR preflight checks overlap risks (and optional dependencies)

Before creating a PR or enabling auto-merge, the agent MUST run:
- `scripts/agent_pr_preflight.sh`

The preflight MUST:
- detect file overlap with other open PRs (conflict risk)

If a roadmap execution plan exists, the preflight SHOULD additionally detect blocked dependencies.
