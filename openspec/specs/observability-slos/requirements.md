# 可观测性与SLO（Observability and SLOs）

## Purpose

本 spec 定义 LS 的「可观测性与SLO」能力（`observability-slos`），用于规范 指标/日志/链路追踪与 SLO/告警 的契约、门禁与审计。
关键产物包括：指标命名与标签规范、日志字段规范、trace 传播规范、SLO/告警规则。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「可观测性与SLO」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `request_id`, `trace_id`, `version_id`, `engine_id`, `user_id`）及其语义边界。
...
系统 MUST 对关键产物（指标命名与标签规范、日志字段规范、trace 传播规范、SLO/告警规则）定义字段级约束，并对完整性/引用可解析性实施校验。
...
系统 SHALL 定义可执行的验收口径，且未达标 MUST 阻断进入其所属门禁（Gate-0）。
