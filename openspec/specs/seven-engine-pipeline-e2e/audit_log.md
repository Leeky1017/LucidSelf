# 七引擎流水线E2E — 审计日志（字段/脱敏/导出结构）

本文件定义 `seven-engine-pipeline-e2e` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | RUN_START/RUN_END/HANDOFF_VALIDATE/RECOVERY_ACTION/AUDIT_EXPORT |
| `user_id` | 否 | 用户标识（若适用；隔离边界） |
| `org_id` | 否 | 组织标识（若适用） |
| `engine_id` | 是 | 引擎标识 |
| `version_id` | 是 | 版本标识 |
| `corpus_release_id` | 是 | 语料发布标识 |
| `run_id` | 否 | E2E 执行标识（若适用） |
| `stage_id` | 否 | 阶段标识（若适用） |
| `from_stage_id` | 否 | 上游阶段标识（若适用） |
| `to_stage_id` | 否 | 下游阶段标识（若适用） |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、完整阶段输出、原始引用片段等）。
