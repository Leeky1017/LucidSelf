# 七引擎流水线E2E（Seven Engine Pipeline E2E）

## Purpose

本 spec 定义 LS 的「七引擎流水线E2E」能力（`seven-engine-pipeline-e2e`），用于规范 七引擎端到端流程的阶段契约、交接校验与可观测 的契约、门禁与审计。
关键产物包括：阶段契约、handoff 校验记录、E2E trace、失败恢复记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「七引擎流水线E2E」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `engine_id`, `version_id`, `corpus_release_id`）及其语义边界。
...
系统 MUST 对关键产物（阶段契约、handoff 校验记录、E2E trace、失败恢复记录）定义字段级约束，并对完整性/引用可解析性实施校验。
...
系统 SHALL 定义可执行的验收口径，且未达标 MUST 阻断进入其所属门禁（Gate-2）。
