# 知识图谱运行时语义查询 Acceptance

## 验收命令（最低集合）
- `openspec validate --specs --strict --no-interactive`
- `openspec list --specs`（确认本 spec 的任务进度可见）
- `.venv/bin/python -c 'import json; json.load(open(\"openspec/specs/knowledge-graph-runtime-semanticquery/contract/knowledge_graph_runtime_semanticquery_contract.v1.json\")); print(\"OK\")'`（契约机器可读校验）
...
## 必须产物（最低集合）
- `openspec/specs/knowledge-graph-runtime-semanticquery/spec.md`
- `openspec/specs/knowledge-graph-runtime-semanticquery/requirements.md`
- `openspec/specs/knowledge-graph-runtime-semanticquery/tasks.md`
