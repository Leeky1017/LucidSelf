# 滥用防护与限流（Abuse and Rate Limits）

## Purpose

本 spec 定义 LS 的「滥用防护与限流」能力（`abuse-and-rate-limits`），用于规范 滥用检测、限流与强制执行策略 的契约、门禁与审计。
关键产物包括：限流策略、滥用信号证据、封禁/挑战/降级动作记录、复核记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「滥用防护与限流」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `org_id`, `engine_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（限流策略、滥用信号证据、封禁/挑战/降级动作记录、复核记录）定义字段级约束，并对完整性/引用可解析性实施校验。
