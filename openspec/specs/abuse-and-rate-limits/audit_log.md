# 滥用防护与限流 — 审计日志（字段/脱敏/导出结构）

本文件定义 `abuse-and-rate-limits` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | RATE_LIMIT_EVALUATE/ABUSE_SIGNAL_DETECTED/ENFORCEMENT_ACTION/REVIEW_DECISION/AUDIT_EXPORT |
...
| `policy_id` | 否 | 关联策略标识（若适用） |
| `evidence_id` | 否 | 关联证据标识（若适用） |
| `action_id` | 否 | 关联强制动作标识（若适用） |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、原始请求 payload）。
