# 语义抽取质量门禁（Semantic Extraction Quality Gates）

## Purpose

本 spec 定义 LS 的「语义抽取质量门禁」能力（`semantic-extraction-quality-gates`），用于规范 语义抽取输出的质量指标、阈值与阻断策略 的契约、门禁与审计。
关键产物包括：质量指标口径、阈值配置、失败样本清单、复检记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「语义抽取质量门禁」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`, `corpus_release_id`）及其语义边界。
...
系统 MUST 对关键产物（质量指标口径、阈值配置、失败样本清单、复检记录）定义字段级约束，并对完整性/引用可解析性实施校验。
