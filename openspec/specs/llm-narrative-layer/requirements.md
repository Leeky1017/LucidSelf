# LLM叙述层（LLM Narrative Layer）

## Purpose

本 spec 定义 LS 的「LLM叙述层」能力（`llm-narrative-layer`），用于规范 叙述输出的结构化格式、安全策略与引用融合 的契约、门禁与审计。
关键产物包括：输出 schema、模型/提示版本记录、引用绑定、内容安全审计。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「LLM叙述层」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `engine_id`, `version_id`, `prompt_id`, `model_version_id`）及其语义边界。
...
系统 MUST 对关键产物（输出 schema、模型/提示版本记录、引用绑定、内容安全审计）定义字段级约束，并对完整性/引用可解析性实施校验。
