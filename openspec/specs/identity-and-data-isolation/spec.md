# 身份与数据隔离（Identity and Data Isolation）

## Purpose

本 spec 定义 LS 的「身份与数据隔离」能力（`identity-and-data-isolation`），用于规范 user_id/org_id 身份范围与多租户隔离 的契约、门禁与审计。
关键产物包括：身份声明、访问控制决策记录、隔离违规审计、删除/保留审计。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「身份与数据隔离」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `org_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（身份声明、访问控制决策记录、隔离违规审计、删除/保留审计）定义字段级约束，并对完整性/引用可解析性实施校验。
