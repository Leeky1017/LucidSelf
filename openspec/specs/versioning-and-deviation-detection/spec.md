# 版本化与偏差检测（Versioning and Deviation Detection）

## Purpose

本 spec 定义 LS 的「版本化与偏差检测」能力（`versioning-and-deviation-detection`），用于规范 版本标识(version_id)与偏差/漂移检测 的契约、门禁与审计。
关键产物包括：版本清单、偏差报告、兼容性声明、阻断记录。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「版本化与偏差检测」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `version_id`, `engine_id`）及其语义边界。
...
系统 MUST 对关键产物（版本清单、偏差报告、兼容性声明、阻断记录）定义字段级约束，并对完整性/引用可解析性实施校验。
