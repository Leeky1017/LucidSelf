# Playbook与行动闭环 — 审计日志（字段/脱敏/导出结构）

本文件定义 `playbook-and-action-loop` 的审计日志最小契约：字段集合、脱敏规则与导出结构。禁止写实现细节；仅定义 MUST/SHALL 与可判定口径。
...
| `event_type` | 是 | PLAYBOOK_CREATE/PLAYBOOK_UPDATE/EXECUTION_START/EXECUTION_FINISH/APPROVAL_DECISION/ROLLBACK_EXECUTE/AUDIT_EXPORT |
| `user_id` | 否 | 用户标识（隔离边界；若适用） |
| `version_id` | 是 | 版本标识 |
| `result` | 是 | PASS/DENY/BLOCK |
| `rejection_reason_code` | 否 | 拒绝/阻断原因代码（见契约枚举） |
| `playbook_id` | 否 | 关联 playbook（若适用） |
| `execution_id` | 否 | 关联执行（若适用） |
| `approval_id` | 否 | 关联审批（若适用） |
| `rollback_id` | 否 | 关联回滚（若适用） |
...
- 审计记录 MUST 不包含不必要的敏感原文（例如：密钥、token、用户自由文本、行动细节原文等）。
- `actor_id`/`user_id` 若属于敏感标识，SHOULD 进行脱敏或不可逆摘要；但 MUST 保持可审计关联性。
