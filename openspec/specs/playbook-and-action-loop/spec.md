# Playbook与行动闭环（Playbook and Action Loop）

## Purpose

本 spec 定义 LS 的「Playbook与行动闭环」能力（`playbook-and-action-loop`），用于规范 行动剧本执行、审批、人机协作与回滚闭环 的契约、门禁与审计。
关键产物包括：playbook 定义、执行记录、审批记录、回滚记录、审计日志。
本 spec 只定义 MUST/SHALL 的行为与验收口径，不包含实现细节/伪代码/代码路径。

## Requirements

### Requirement: 契约与标识符（Contract & IDs）
系统 MUST 为「Playbook与行动闭环」定义稳定、版本化契约，并明确最小标识符集合（至少包含 `user_id`, `version_id`）及其语义边界。
...
系统 MUST 对关键产物（playbook 定义、执行记录、审批记录、回滚记录、审计日志）定义字段级约束，并对完整性/引用可解析性实施校验。
