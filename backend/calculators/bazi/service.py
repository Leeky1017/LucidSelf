# backend/calculators/bazi/service.py
"""
BaziCalculator Service Layer - Geocoding 集成入口.
...
from backend.calculators.bazi.models import BaziInput, BaziFactors
...
async def calculate_with_geocoding(
    ...
    此函数是 BaziCalculator 的 Geocoding 集成入口：
    1. 如果提供了 birth_location，直接使用
    2. 如果只提供了 birth_place，通过 GeocodingService 解析
    3. 解析后调用 BaziCalculator.calculate()
    
    Args:
        input_data: 八字计算输入，支持 birth_place 或 birth_location
