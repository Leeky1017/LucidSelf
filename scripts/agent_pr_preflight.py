...
def _controlplane_guard(repo_root: Path) -> None:
    ...
    if controlplane_dirty:
        print(
            ...
            f"{controlplane_dirty}\nFix: keep controlplane main clean; move changes into a task worktree."
        )
        raise RuntimeError("controlplane dirty")


def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LS agent PR preflight: overlap (and optional deps).")
    parser.add_argument(
        "--base-ref",
        default="origin/main",
